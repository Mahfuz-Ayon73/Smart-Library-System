...
async function IssueBook(req, res) {
    try {
        ...
        const bookResponse = await axios.get(`http://books-service:8081/api/books/getbook/${book_id}`);
        ...
        await axios.patch(`http://books-service:8081/api/books/update/${book_id}`,
            {
...
async function ReturnBook(req, res) {
    try {
        ...
        if (updatedLoan) {
            ...
            const bookResponse = await axios.get(`http://books-service:8081/api/books/getbook/${updatedLoan.book_id}`);

            const book = bookResponse.data;

            res.status(200).json(updatedLoan);
            await axios.patch(`http://books-service:8081/api/books/update/${book._id}`,
                {
                    available_copies: book.available_copies + 1
                }
            );
    ...
    try {
        ...
        if (allLoans) {
            ...
            for (let loan of allLoans) {
                ...
                const foundBook = await axios.get(`http://books-service:8081/api/books/getbook/${loan.book_id}`);
    ...
    try {
        ...
        for (let loan of allLoans) {
            if (loan.due_date < currentData) {
                let foundUser = await axios.get(`http://users-service:8080/api/users/retrieve/${loan.user_id}`);
                let user = foundUser.data;
                let foundBook = await axios.get(`http://books-service:8081/api/books/getbook/${loan.book_id}`);
